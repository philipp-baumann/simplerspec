...
evaluate_model <- function(data, obs, pred) {
  ...
  tibble::tibble(
    ...
    me = mean(obs, na.rm = TRUE) - mean(pred, na.rm = TRUE),
    bias = mean(obs, na.rm = TRUE) - mean(pred, na.rm = TRUE),
    ...
    b = lm(obs ~ y)$coefficients[2],
    # Ratio of performance to deviation
    rpd = sd(obs, na.rm = TRUE) /
      sqrt(sum((obs - pred)^2, na.rm = TRUE) / (length(obs) - 1)),
    ...
    # Non-unity slope
    NU = mean((pred - mean(pred))^2) * (1 - lm(obs ~ y)$coefficients[2])^2,
    # Lack of correlation
    ...
    NU_prop = round(mean((pred - mean(pred))^2)
      * (1 - lm(obs ~ y)$coefficients[2])^2 / mean((pred - obs)^2) * 100, 0),
